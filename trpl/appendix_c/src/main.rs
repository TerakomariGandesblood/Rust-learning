...
// Eq trait 表明每一个被标记类型的值等于其自身（例如两个 NaN 互不相等），只能应用于那些实现了 PartialEq 的类型
// 对于 HashMap<K, V> 中的 key 来说，Eq 是必须的

// PartialOrd trait，可以使用 <、 >、<= 和 >= 运算符，实现 partial_cmp 方法，只能在实现了 PartialEq 的类型上实现
// Ord trait 表明任意两个值存在有效顺序，实现了 cmp 方法（例如两个 NaN 无法产生顺序），只可以在实现了 PartialOrd 和 Eq 的类型上实现 Ord trait
// 在 BTreeSet<T>上存值时，Ord 是必须的
