...
struct Screen {
    ...
    // trait 对象指向一个实现了我们指定 trait 的类型的实例
    // 以及一个用于在运行时查找该类型的 trait 方法的表，只有符合 dyn 兼容性（dyn compatibility）的
    // trait 可以实现为 trait 对象
    // 见 https://doc.rust-lang.org/reference/items/traits.html#dyn-compatibility
