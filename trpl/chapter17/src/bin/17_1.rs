// 异步编程正是一种抽象，它让我们能够用“可能暂停的点”和“最终结果”来表达代码，而协调执行的细节则由它来替我们处理
...
// async 块或 async 函数中每一个等待 future 的地方都可能是一个 async 块或 async 函数中断并随后恢复的点

// Rust 中的 futures 是 惰性（lazy）的：在你使用 await 请求之前它们不会执行任何操作

// 当 Rust 遇到一个 async 关键字标记的代码块时，会将其编译为一个实现了 Future trait 的唯一的、匿名的 future
...
// Rust 需要记录异步代码块中涉及的状态，这样运行时可以去执行其他工作并在准备好时回来继续推进当前的任务
